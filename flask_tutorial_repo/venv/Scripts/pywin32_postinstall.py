...
def install(lib_dir):
    ...
    for dest_dir in [get_system_dir(), sys.prefix]:
        ...
        try:
            for first_name in files:
                base = os.path.basename(first_name)
                dst = os.path.join(dest_dir, base)
                CopyTo("installing %s" % base, first_name, dst)
                if verbose:
    ...
    for first_name in glob.glob(os.path.join(pywin_dir, "*.cfg")):
        file_created(first_name[:-1] + "c")  # .cfg->.cfc
...
def uninstall(lib_dir):
    ...
    try:
        ...
        pywin_dir = os.path.join(lib_dir, "Pythonwin", "pywin")
        for first_name in glob.glob(os.path.join(pywin_dir, "*.cfc")):
